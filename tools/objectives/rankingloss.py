# copy from https://github.com/zifyloo/SSAN/blob/main/src/loss/RankingLoss.py
...
def calculate_margin_cr(similarity_match_cr, similarity_match, auto_margin_flag, margin):
    if auto_margin_flag:
        ...
        data = torch.ge(ones, lambda_cr).float()  # 比较大小 a>=b, 比1大的为1，比1小的为0
        data_2 = torch.ge(lambda_cr, ones).float() #
        lambda_cr = data * lambda_cr + data_2  # 最终效果min(x,1)：lambda_cr比1大的地方取1，比1小的取本身
...
class CRLoss(nn.Module):
    ...
    def forward(self, img, txt, txt_cr, labels, auto_margin_flag):
        '''
        :param img: 图像全局表征
        :param txt: 图像对应的正样本文本全局表征
        :param txt_cr: 图像对应的一个弱样本文本的全局表征
        :param labels: 图像对应的行人ID号码

        :param auto_margin_flag: 是否使用动态软标签
        :return:
        '''
