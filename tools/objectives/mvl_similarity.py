...
# copy from  https://github.com/kdwonn/DivE/blob/main/similarity.py
...
class SetwiseSimilarity(nn.Module):
    def __init__(self, img_set_size, txt_set_size, denominator=2, temperature=1):
    ...
    def smooth_chamfer_similarity_euclidean(self, img_embs, txt_embs):
        ...
        smooth_chamfer_dist = (first_term / (self.img_set_size * self.temperature) + second_term / (
                    self.txt_set_size * self.temperature)) / (self.denominator)
    ...
    def smooth_chamfer_similarity_cosine(self, img_embs, txt_embs):
        ...
        first_term = self.y_axis_sum_pool(
            torch.log(self.x_axis_sum_pool(
                torch.exp(self.temperature * dist.unsqueeze(0))   # 这个地方显存占用很多
            ))).squeeze(0)
        second_term = self.x_axis_sum_pool(
            torch.log(self.y_axis_sum_pool(
                torch.exp(self.temperature * dist.unsqueeze(0))
        ...
        smooth_chamfer_dist = (first_term / (self.img_set_size * self.temperature) + \
                                      second_term / (self.txt_set_size * self.temperature)
                              ) / (self.denominator)

        return smooth_chamfer_dist

    def smooth_chamfer_similarity_cosine_return2(self, img_embs, txt_embs):
        """
            cosine version of smooth_chamfer_similarity_euclidean(img_embs, txt_embs)
        """
        dist = cosine_sim(img_embs, txt_embs)
        ...
        first_dist = first_term / (self.img_set_size * self.temperature)
        second_dist = second_term / (self.txt_set_size * self.temperature)
